    ...
    public class DataStore : Model, IStorageReader, IStorageWriter, IDisposable
    {
        ...
        public bool Open(bool readOnly)
        {
            ...
            else
            {
                ...
                if (!connection.TableExists("_Simulations"))
                    connection.ExecuteNonQuery("CREATE TABLE _Simulations(ID INTEGER generated by default as identity PRIMARY KEY NOT NULL ASC, Name VARCHAR(50) COLLATE UNICODE_CI, FolderName VARCHAR(50) COLLATE UNICODE_CI)");
                if (!connection.TableExists("_Messages"))
